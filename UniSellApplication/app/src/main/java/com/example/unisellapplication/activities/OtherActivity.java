...
public class OtherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar = findViewById(R.id.other_toolbar);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        //Recent items recycle
        recentRecycle.setHasFixedSize(true);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager( OtherActivity.this);
        linearLayoutManager.setReverseLayout(true);
        linearLayoutManager.setStackFromEnd(true);
        recentRecycle.setLayoutManager(linearLayoutManager);

        listingModelList = new ArrayList<>();
        recentAdapters = new RecentAdapters( OtherActivity.this, listingModelList, new RecentAdapters.OnItemClickListener() {
            @Override
            public void onItemClick(ListingModel listItem) {
                Toast.makeText( OtherActivity.this, "Item Clicked", Toast.LENGTH_LONG).show();
            }
        });
        recentRecycle.setAdapter(recentAdapters);

        addListingReference.orderByChild("category").equalTo("Other").addValueEventListener(new ValueEventListener() {
            @Override
    ...
    private void searchProduct(String inText) {
        ...
        filteredAdapter = new RecentAdapters(OtherActivity.this, filteredList, new RecentAdapters.OnItemClickListener() {
            @Override
            public void onItemClick(ListingModel listItem) {
                Toast.makeText(OtherActivity.this, "Item Clicked", Toast.LENGTH_LONG).show();
            }
        ...
        if(!inText.isEmpty()){
            ...
            if(filteredList.isEmpty()){
                Toast.makeText(OtherActivity.this, "Search results not found", Toast.LENGTH_SHORT).show();
            }
            else {
